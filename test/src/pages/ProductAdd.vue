  ...
  methods:{
    ...
    async submitForm() {
      if (!this.isInvalid && this.validateForm() && this.checkSKU(this.product.SKU)) {
        if (this.isDVD === true) {
          try {
            await fetch(`http://localhost:8080/?addDVD&sku=${this.product.SKU}&name=${this.product.name}&price=${this.product.price}&size=${this.product.size}`);
            setTimeout(() => {
                this.$router.push('/');
              }, 500);
          } catch (error) {
        ...
        if (this.isBook === true) {
          try {
            await fetch(`http://localhost:8080/?addBook&sku=${this.product.SKU}&name=${this.product.name}&price=${this.product.price}&weight=${this.product.weight}`);
            setTimeout(() => {
                this.$router.push('/');
              }, 500);
          } catch (error) {
        ...
        if (this.isFurniture === true) {
          try {
            await fetch(`http://localhost:8080/?addFurniture&sku=${this.product.SKU}&name=${this.product.name}&price=${this.product.price}&dimension=${this.product.height}x${this.product.width}x${this.product.length}`);
            setTimeout(() => {
                this.$router.push('/');
              }, 500);
          } catch (error) {
